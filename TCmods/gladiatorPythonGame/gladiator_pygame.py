...
class LD_stuff:
    ...
    def Thief(Player,player):
        """Ternary"""
        """ing java or even javascript:
        color = warm>70 ? "red" : "blue";
...
"""
    ...
    def Archer(Player,player):
        ...
        return proj
    def LD2(Player,player):
        facing = player.facing if player.facing.length_squared() > 0 else Vec(1, 0)
        pos = player.pos + facing * (player.radius + 4)
        vel = facing * player.proj_speed
        proj = Projectile(pos=pos, vel=vel, radius=100, damage=player.damage, lifetime=max(0.4, player.proj_range / max(100.0, player.proj_speed)), pierce=1000, color=(255,255,255), owner_id=player.pid)
        return proj
    def LD3(Player,player):
        facing = player.facing if player.facing.length_squared() > 0 else Vec(1, 0)
        pos = player.pos + facing# * (player.radius + 4)
        vel = facing * player.proj_speed * 0
        proj = Projectile(pos=pos, vel=vel, radius=200, damage=player.damage, lifetime=max(0.4, player.proj_range / max(100.0, player.proj_speed)), pierce=1000, color=(255,255,255), owner_id=player.pid)
        return proj
    def Mage(Player,player):
        facing = player.facing if player.facing.length_squared() > 0 else Vec(1, 0)
        pos = player.pos + facing * (player.radius + 4)
        ...
        return proj
    atks={"Thief":Thief,"Archer":Archer,"Mage":Mage}
    class_atks={"Thief":atks["Thief"],"Archer":atks["Archer"],"Mage":atks["Mage"]}
def create_class(name,stats=CLASS_PRESETS["Thief"],atk=LD_stuff.Thief):
    if type(atk)==str:
        LD_stuff.class_atks[name]=LD_stuff.atks[atk]
    else:
...
def create_atk(name,starting_dist,proj_speed_multiplier,min_proj_range,lifetime_multiplier,damage_multiplier,proj_size,pierce,color):
    def atk(Player,player):
        facing = player.facing if player.facing.length_squared() > 0 else Vec(1, 0)
        pos = player.pos + facing * (player.radius + starting_dist)
        vel = facing * player.proj_speed * proj_speed_multiplier
        proj = Projectile(pos=pos, vel=vel, radius=proj_size, damage=player.damage * damage_multiplier, lifetime=player.proj_range / max(min_proj_range, player.proj_speed * proj_speed_multiplier) * lifetime_multiplier, pierce=pierce, color=color, owner_id=player.pid)
        return proj
...
def player_attack(player: Player, now: float) -> List[Projectile]:
    ...
    shots.append(LD_stuff.class_atks[player.class_type](Player,player))
    return shots
...
def create_upgrade(name,att,amount,mesg,typ="+"):
    """(name:  id),
    (att:  attribute can be 'max_hp','max_shield','color','speed','damage',
    'fire_rate','proj_range','max_shield','shield'),
    (amount:  how much to add to att),(mesg:  the mesg to show on the button),
    (typ="+",set typ to "set" to set a value)
    """
...
class Game:
    ...
    def update_levelup(self, dt):
        ...
        if p1_confirm and p2_confirm:
            for p in self.players:
                title, attr, delta, desc, typ = self.levelup_options[p.pid][self.levelup_selected[p.pid]]
...
def new_mage():
    create_atk('new_mage',100,0,0.001,0.000001,4,90,100,GREEN)
    create_class('new_mage',CLASS_PRESETS['Mage'],"new_mage")
def new_mage():
    CLASS_PRESETS["new_mage"]=CLASS_PRESETS["Mage"]
    CLASS_PRESETS["new_mage"]["fire_rate"]=0.5
    create_atk('new_mage',100,0,0.01,0.00005,0.1,90,1000,GREEN)
    create_class('new_mage',CLASS_PRESETS['new_mage'],"new_mage")
def upgrades():
    create_upgrade('curse of deniabl speed','speed',99999999999999999999999999999999,'click if you dare!!!')
